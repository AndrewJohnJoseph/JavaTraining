...
begin
 ...
 --  er.vsal :=10000;
      ...
      select empno,ename,sal into er.vid,er.vname, er.vsal from sys.emp where empno=7369;
...
create table emp2 as select * from sys.emp;
...
declare
    emprec emp%rowtype;
    cursor mynewcur (jobtype varchar2) is select * from emp where job=jobtype;
begin
    open mynewcur('MANAGER');
    loop
    fetch mynewcur into emprec;
    exit when mynewcur%NOTFOUND;    
    dbms_output.put_line(emprec.ename || '    ' || emprec.sal); 
    end loop;
    close mynewcur;
    open mynewcur('ANALYST');
    loop
    fetch mynewcur into emprec;
    exit when mynewcur%NOTFOUND;    
    dbms_output.put_line(emprec.ename || '    ' || emprec.sal); 
...
declare
    emprec emp%rowtype;
    cursor mynewcur (jobtype1 varchar2, jobtype2 varchar2) is select * from emp where job=jobtype1 or job=jobtype2;
begin
    open mynewcur('MANAGER', 'ANALYST');
    loop
    fetch mynewcur into emprec;
    exit when mynewcur%NOTFOUND;    
    dbms_output.put_line(emprec.ename || '    ' || emprec.sal); 
...
declare
    ...
    revnumber Number := 0;
begin
    newnumber := TO_CHAR(&no);
    
    while newnumber>0 loop
        revnumber := (revnumber * 10) + mod(newnumber,10);
        newnumber := floor(newnumber / 10);
    end loop;
       dbms_output.put_line(revnumber);
end;
/
...
--create table batch210 (stid number, sname varchar2(10), deptno number)
declare
  eno emp.empno%type;
  enam emp.ename%type;
  salry emp.sal%type;
...
begin
    select empno,ename,sal into eno,enam,salry from sys.emp where empno=&no;
