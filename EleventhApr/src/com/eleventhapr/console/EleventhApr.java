...
//		 thenAcceptBothMethod();
	...
	private static void handleException() {
		...
		private static void allOfMethod() {
			...
			CompletableFuture<Integer> cFuture2 = CompletableFuture.supplyAsync(() -> 50);
			CompletableFuture<String> cFuture3 = CompletableFuture.supplyAsync(() -> "Deeps");
			CompletableFuture<Employee> cFuture4 = CompletableFuture.supplyAsync(() -> new Employee(20, "test", "test"));
			...
			combinedFuture.thenApply(
					v -> Stream.of(cFuture1, cFuture2, cFuture3, cFuture4).map(CompletableFuture::join).collect(Collectors.toList())).thenAccept((lstRes) -> {
						lstRes.forEach(res -> {
